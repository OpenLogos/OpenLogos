...
{
public:
   static CLgsOut &singleton();
   ~CLgsOut() ; //destructor

   void setLogFile(const LgsString &fileName);
   bool AppendFile(const LgsString &fileName);
   const LgsString &getFileName() const;
   
   CLgsOut &operator<<(LgsString &s) ;
   CLgsOut &operator<<(const LgsString &s) ;
...
private:
   ...
   LgsString m_fileName;
   ofstream *pLogFile ;
   static CLgsOut m_logStream;
};
//---------------------------------------------------------------------
inline const LgsString &CLgsOut::getFileName() const
{
   return m_fileName;
}
