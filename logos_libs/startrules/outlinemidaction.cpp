...
void ST_OutlineMidAction::fire(ST_Variable& variable) const
{
    bool lp = variable.getSubString(1).length() != 0;
    dump(lp);
    char text = variable.getSubString(2)[0];
    dump(text);
    ...
    if (lp && rp)
        variable.setTokenType(LookupTokenType::tok_Enumeration, true);
}
