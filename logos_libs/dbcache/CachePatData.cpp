...
CachePatData::CachePatData(SqlConnection *con, int langcode, bool loader, bool fromFile,
			 bool saveIt) :
	CacheData(con, langcode, "Pat", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_row);
	max_key_len = O_PN_NC_CC_KEYLEN;
...
CachePatData::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_nc_cc size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	stmt = (SqlStatement *)oracleConnection->CreateStatement();
	stmtText = "select pat_number, number_code, case_code, "
		" stem_number, ending "
		" from pat_table "
		" where language_code = :lc "
		" order by pat_number, number_code, case_code, stem_number";
	...
	SqlColumn *cc=stmt->BindOutputColumn(3,SqlColumn::Integer);
...
//fflush(stdout);
		struct o_row *lrp = getRow(j);
		makeKey(key, lrp->pat_number, lrp->number_code, lrp->case_code, j);
		hashtab->put(key);
	}
...
void
CachePatData::makeKey(CacheKey *key, int pn, int nc, int cc, int val) {
