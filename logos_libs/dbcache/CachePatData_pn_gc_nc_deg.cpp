...
CachePatData_pn_gc_nc_deg::CachePatData_pn_gc_nc_deg(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_gc_nc_deg", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_gc_nc_deg_row);
	max_key_len = O_PN_GC_NC_DEG_KEYLEN;

	mapAndLoad();
}

void
CachePatData_pn_gc_nc_deg::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_gc_nc_deg size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData_pn_gc_nc_deg::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	SqlColumn *deg=stmt->BindOutputColumn(4,SqlColumn::Integer);
...
// first load the data and calculate weights
	...
	while(stmt->Fetch()) {
		struct o_pn_gc_nc_deg_row *lrp = getRow(nrows);
...
//fflush(stdout);
		struct o_pn_gc_nc_deg_row *lrp = getRow(j);
...
void
CachePatData_pn_gc_nc_deg::makeKey(CacheKey *key, int pn, int gc, int nc,
								   int dg, int val) {
