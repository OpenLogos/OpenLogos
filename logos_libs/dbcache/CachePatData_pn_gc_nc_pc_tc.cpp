...
CachePatData_pn_gc_nc_pc_tc::CachePatData_pn_gc_nc_pc_tc(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_gc_nc_pc_tc", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_gc_nc_pc_tc_row);
	max_key_len = O_PN_GC_NC_PC_TC_KEYLEN;

	mapAndLoad();
}

void
CachePatData_pn_gc_nc_pc_tc::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_gc_nc_pc_tc size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData_pn_gc_nc_pc_tc::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	stmtText = "select pat_number, gender_code, number_code, "
		" person_code, tense_code, stem_number, ending "
		" from pat_table "
		" where language_code = :lc "
		" order by pat_number,gender_code,number_code,person_code, "
		" tense_code";
	...
	SqlColumn *pc=stmt->BindOutputColumn(4,SqlColumn::Integer);
	SqlColumn *tc=stmt->BindOutputColumn(5,SqlColumn::Integer);
	SqlColumn *sn=stmt->BindOutputColumn(6,SqlColumn::Integer);
	SqlColumn *ending=stmt->BindOutputColumn(7, SqlColumn::StringType);
...
// first load the data and calculate weights
	...
	while(stmt->Fetch()) {
		struct o_pn_gc_nc_pc_tc_row *lrp = getRow(nrows);
		lrp->pat_number = pn->AsInteger();
		lrp->gender_code = gc->AsInteger();
		lrp->number_code = nc->AsInteger();
		lrp->person_code = pc->AsInteger();
		lrp->tense_code = tc->AsInteger();
...
// (pat_numer, gender_code, number_code, person_code, tense_code)
		...
		cp = (unsigned char *) & lrp->person_code;
		for(i=0;i<sizeof(int);i++) {
			int val = *cp;
			weights[val]++;
			nletters++;
		...
		cp = (unsigned char *) & lrp->tense_code;
		for(i=0;i<sizeof(int);i++) {
			int val = *cp;
			weights[val]++;
			nletters++;
...
//fflush(stdout);
		struct o_pn_gc_nc_pc_tc_row *lrp = getRow(j);
		makeKey(key, lrp->pat_number, lrp->gender_code,
			lrp->number_code, lrp->person_code,
			lrp->tense_code, j);
		hashtab->put(key);
	}
...
void
CachePatData_pn_gc_nc_pc_tc::makeKey(CacheKey *key, int pn, int gc, int nc,
		int pc, int tc, int val) {
