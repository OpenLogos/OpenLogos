...
CachePatData_pn_gc_nc_cc::CachePatData_pn_gc_nc_cc(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_gc_nc_cc", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_gc_nc_cc_row);
	max_key_len = O_PN_GC_NC_CC_KEYLEN;

	mapAndLoad();
}

void
CachePatData_pn_gc_nc_cc::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_gc_nc_cc size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData_pn_gc_nc_cc::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	SqlColumn *cc=stmt->BindOutputColumn(4,SqlColumn::Integer);
...
// first load the data and calculate weights
	...
	while(stmt->Fetch()) {
		struct o_pn_gc_nc_cc_row *lrp = getRow(nrows);
...
//fflush(stdout);
		struct o_pn_gc_nc_cc_row *lrp = getRow(j);
		makeKey(key, lrp->pat_number,lrp->gender_code, lrp->number_code, 
			lrp->case_code, j);
		hashtab->put(key);
	}
...
void
CachePatData_pn_gc_nc_cc::makeKey(CacheKey *key, int pn, int gc, int nc, 
								  int cc, int val) {
