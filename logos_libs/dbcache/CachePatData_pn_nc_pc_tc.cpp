...
CachePatData_pn_nc_pc_tc::CachePatData_pn_nc_pc_tc(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_nc_pc_tc", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_nc_pc_tc_row);
	max_key_len = O_PN_NC_PC_TC_KEYLEN;
...
fflush(stdout);
		...
		for(int j=0;j<size;j++) {
			struct o_pn_nc_pc_tc_row *lrp = getRow(j);
			char keybuf[256];
			CacheKey *key = (CacheKey *)keybuf;
			makeKey(key, lrp->pat_number, lrp->number_code, 
				lrp->person_code, lrp->tense_code, 0);
printf("\nrow # %d, %d %d %d %d %d \"%s\": keyval=%d\n", j,
		   lrp->pat_number, lrp->number_code, lrp->person_code,
		   lrp->tense_code, lrp->stem_number, lrp->ending, j);
fflush(stdout);
		}
//		for(j=0;j<256;j++) {
//printf("weight[%d] = %f\n", j, weights_array[j]);
...
void
CachePatData_pn_nc_pc_tc::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_nc_pc_tc size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData_pn_nc_pc_tc::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	stmt = (SqlStatement *)oracleConnection->CreateStatement();
	stmtText = "select pat_number, number_code, person_code,"
		" tense_code, stem_number, ending "
		" from pat_table "
		" where language_code = :lc "
		" order by pat_number,number_code,person_code,tense_code,stem_number";
	...
	SqlColumn *pc=stmt->BindOutputColumn(3,SqlColumn::Integer);
	SqlColumn *tc=stmt->BindOutputColumn(4,SqlColumn::Integer);
	SqlColumn *sn=stmt->BindOutputColumn(5,SqlColumn::Integer);
	SqlColumn *ending=stmt->BindOutputColumn(6,
...
// first load the data and calculate weights
	CacheKey *key = (CacheKey *)keybuf;
	while(stmt->Fetch()) {
		struct o_pn_nc_pc_tc_row *lrp = getRow(nrows);
...
// calculating frequencies, key is (pat_numer, number_code, case_code)
...
//fflush(stdout);
		struct o_pn_nc_pc_tc_row *lrp = getRow(j);
		makeKey(key, lrp->pat_number, lrp->number_code, 
			lrp->person_code, lrp->tense_code, j);
		hashtab->put(key);
	}
...
void
CachePatData_pn_nc_pc_tc::makeKey(CacheKey *key, int pn,int nc,int pc,
								  int tc,int val) {
