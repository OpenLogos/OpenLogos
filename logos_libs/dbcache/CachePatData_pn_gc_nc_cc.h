...
*/
#ifndef _CachePatData_pn_gc_nc_ccClass
#define _CachePatData_pn_gc_nc_ccClass

#define O_NEN 5017
#define O_NDE 40232
#define O_NFR 9516

#define O_PN_GC_NC_CC_KEYLEN 28
		// sizeof(pat_number) + sizeof(gender_code)
		// sizeof(number_code) + sizeof(case_code) + 3 * sizeof(int)
...
class DLLEXPORT CachePatData_pn_gc_nc_cc : public CacheData {

public:

	CachePatData_pn_gc_nc_cc(SqlConnection *con, int langcode, bool loader,
		bool ff, bool sav);
	void makeKey(CacheKey *, int pn, int gc, int nc, int case_code, int val);
	virtual bool 	populateFromDatabase();
	virtual void getSizeFromDB();
	inline struct o_pn_gc_nc_cc_row * getRow(int i) {
		struct o_pn_gc_nc_cc_row *lrp = 
		(struct o_pn_gc_nc_cc_row *)((char *)data_array + i*sizeof(struct o_pn_gc_nc_cc_row));
		return lrp;
	}
...
#endif // _CachePatData_pn_gc_nc_ccClass
