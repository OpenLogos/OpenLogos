...
CachePatData_pn_gc_nc::CachePatData_pn_gc_nc(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_gc_nc", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_gc_nc_row);
	max_key_len = O_PN_GC_NC_KEYLEN;

	mapAndLoad();
}

void
CachePatData_pn_gc_nc::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_gc_cc size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData_pn_gc_nc::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	stmtText = "select pat_number, gender_code, number_code, "
		...
		" from pat_table "
		" where language_code = :lc "
		" order by pat_number,gender_code,number_code,stem_number";
	...
	SqlColumn *sn=stmt->BindOutputColumn(4,SqlColumn::Integer);
	SqlColumn *ending=stmt->BindOutputColumn(5,
...
// first load the data and calculate weights
	...
	while(stmt->Fetch()) {
		struct o_pn_gc_nc_row *lrp = getRow(nrows);
...
//fflush(stdout);
		struct o_pn_gc_nc_row *lrp = getRow(j);
		makeKey(key, lrp->pat_number,lrp->gender_code,  lrp->number_code, j);
		hashtab->put(key);
	}
...
void
CachePatData_pn_gc_nc::makeKey(CacheKey *key, int pn, int gc, int nc, int val) {
