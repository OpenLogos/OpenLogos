...
CachePatData_pn_gc_nc_pc_cc::CachePatData_pn_gc_nc_pc_cc(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_gc_nc_pc_cc", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_gc_nc_pc_cc_row);
	max_key_len = O_PN_GC_NC_PC_CC_KEYLEN;

	mapAndLoad();
}

void
CachePatData_pn_gc_nc_pc_cc::getSizeFromDB() {
	...
	delete stmt;
//printf("Pat_pn_gc_nc_pc_cc size from DB = %d\n", size);
//fflush(stdout);
}

bool
CachePatData_pn_gc_nc_pc_cc::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	stmtText = "select pat_number, gender_code, number_code, "
		" person_code, case_code, stem_number, ending "
		" from pat_table "
		" where language_code = :lc "
		" order by pat_number,gender_code,number_code,person_code, "
		" case_code, stem_number";
	...
	SqlColumn *ending=stmt->BindOutputColumn(7,
...
// first load the data and calculate weights
	...
	while(stmt->Fetch()) {
		struct o_pn_gc_nc_pc_cc_row *lrp = getRow(nrows);
...
// (pat_numer, gender_code, number_code, person_code, case_code)
...
//fflush(stdout);
		struct o_pn_gc_nc_pc_cc_row *lrp = getRow(j);
...
void
CachePatData_pn_gc_nc_pc_cc::makeKey(CacheKey *key, int pn, int gc, int nc,
		int pc, int cc, int val) {
