...
CachePatData_pn_deg::CachePatData_pn_deg(SqlConnection *con, int langcode,
		bool loader, bool fromFile, bool saveIt) :
	CacheData(con, langcode, "Pat_pn_deg", loader, fromFile, saveIt) {

	row_len = sizeof(struct o_pn_deg_row);
	max_key_len = O_PN_DEG_KEYLEN;

	mapAndLoad();
}

void
CachePatData_pn_deg::getSizeFromDB() {
	...
	delete stmt;
printf("Pat_pn_deg size from DB = %d\n", size);
fflush(stdout);
}

bool
CachePatData_pn_deg::populateFromDatabase() {
	char keybuf[256];
	int weights[256];
	int nletters = 0;
	int printstep = 256;
	...
	stmt = (SqlStatement *)oracleConnection->CreateStatement();
	stmtText = "select pat_number, degree, stem_number, ending "
		" from pat_table "
		" where language_code = :lc "
		" order by pat_number, degree, stem_number";
	...
	SqlColumn *deg=stmt->BindOutputColumn(2,SqlColumn::Integer);
	SqlColumn *sn=stmt->BindOutputColumn(3,SqlColumn::Integer);
	SqlColumn *ending=stmt->BindOutputColumn(4,SqlColumn::StringType);
...
// first load the data and calculate weights
	...
	while(stmt->Fetch()) {
		struct o_pn_deg_row *lrp = getRow(nrows);
...
//fflush(stdout);
		struct o_pn_deg_row *lrp = getRow(j);
		makeKey(key, lrp->pat_number, lrp->degree, j);
		hashtab->put(key);
	}
...
void
CachePatData_pn_deg::makeKey(CacheKey *key, int pn, int dg, int val) {
