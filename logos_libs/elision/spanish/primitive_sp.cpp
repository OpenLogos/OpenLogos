...
bool EL_SP_Primitive::hasAccent() const
{
    for (LgsString::iterator iter = word_.begin(); iter != word_.end(); iter++)
        if (isAccent(*iter))
            return true;
...
void EL_SP_Primitive::placeAccent(int position)
{
...
EL_SP_Primitive::CharType EL_SP_Primitive::GetCharType(LgsString::iterator& c)
{
    switch (*c)
    {
...
bool EL_SP_Primitive::isAccent(char c)
{
    switch (c)
    {
    case 'Á': case 'á':
...
LgsString::iterator EL_SP_Primitive::getDominantVowel(LgsString::iterator iter)
{
    CharType type = GetCharType(iter);
...
void EL_SP_Primitive::placeAccent(LgsString::iterator iter)
{
