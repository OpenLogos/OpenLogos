...
void /*FUNCTION*/ copy44()
{
	...
	/*     Check if K3P2 VC exists in K3P1's OPADRI */

	if( vbdataX_.k3p1 < 0 ){
		phrstr = sworkX_.phrbeg[im81X_.im81-vbdataX_.k3p1-One];
		phrlst = sworkX_.phrend[im81X_.im81-vbdataX_.k3p1-One];
		}
	else{
		phrstr = sworkX_.phrbeg[im81X_.im81+vbdataX_.k3p1-One];
		phrlst = sworkX_.phrend[im81X_.im81+vbdataX_.k3p1-One];
	...
	for( jj=phrstr; jj <= phrlst; jj++ ){
		vc1loc = jj;
		if( opadriX_.opadri[jj-One] == -k3p2 )
			goto L_60;

		/*     VC K3P2 was not found in OPADRI, simply return */

		}
	return;

	/*     VC K3P2 exists in OPADRI. Do the same check for K3P4 */

L_60:
	;

	/*     Check if K3P4 VC exists in K3P3's OPADRI */

	if( k3p3 < 0 ){
		phrstr = sworkX_.phrbeg[im81X_.im81-k3p3-One];
		phrlst = sworkX_.phrend[im81X_.im81-k3p3-One];
		}
	else{
		phrstr = sworkX_.phrbeg[im81X_.im81+k3p3-One];
		phrlst = sworkX_.phrend[im81X_.im81+k3p3-One];
	...
	for( jj=phrstr; jj <= phrlst; jj++ ){
		vc2loc = jj;
		if( opadriX_.opadri[jj-One] == -k3p4 )
			goto L_100;

		/*     VC K3P4 was not found in OPADRI, simply return */
		...
		if( srchfd == 0 ){
			...
			if( vtrfm3 == vtrfvlX_.vtmpp ){
				if( tmany ){
					...
					zapit(&hfdoaX_.hfpoad[tgtadr-One][0],SIZ,(byte)zero);
					zapit(&hfdoaX_.sconhf[tgtadr-One][0],SIZ,(byte)zero);
					}
				else{
					hpdopiX_.hfdopi[vc2loc-One] = 0;
					}
			...
			if( smany && tmany ){
				...
				if( bytlft >= 0 ){
					...
					lmove(&hfdoaX_.hfpoad[adr_-One][nxtpos-One],1,&hfdoaX_.hfpoad[ad1-One][0],1,bytes);
					lmove(&hfdoaX_.sconhf[adr_-One][nxtpos-One],1,&hfdoaX_.sconhf[ad1-One][0],1,bytes);
					hfdoaX_.hfpoad[adr_-One][HFPADX-One] = nxtpos + last1 - 1;
					goto L_200;
					}
				}
			else if( tmany ){
				...
				if( vtrfm3 == vtrfvlX_.vtmrp ){
					...
					adr_ = tgthfd - HFPOL1;
					nxtpos = hfdoaX_.hfpoad[adr_-One][HFPADX-One] + 
					  1;
					...
					if( nxtpos < HFPADX ){
						...
						hfdoaX_.sconhf[adr_-One][nxtpos-One] = opadriX_.sconpi[vc1loc-One];
					...
					opadriX_.sconpi[vc2loc-One] = opadriX_.sconpi[vc1loc-One];
					adr_ = tgthfd - HFPOL1;
					zapit(&hfdoaX_.hfpoad[adr_-One][0],SIZ,(byte)zero);
					zapit(&hfdoaX_.sconhf[adr_-One][0],SIZ,(byte)zero);
					goto L_200;
					}
				}
			else{
				if( smany ){
					...
					if( hfdoaX_.adct <= HFPADY ){
						...
						/*     Move the Target element from HFDOPO to HFPOAD
						 *     only if the target element is not zero and of -8X type */

						if( vtrfm3 == vtrfvlX_.vtmrp && tgthfd != 0 ){
							hfdoaX_.hfpoad[hfdoaX_.adct-One][0] = tgthfd;
							hfdoaX_.sconhf[hfdoaX_.adct-One][0] = opadriX_.sconpi[vc2loc-One];
							nxtpos = 2;
							}
						else{
							nxtpos = 1;
						...
						lmove(&hfdoaX_.hfpoad[hfdoaX_.adct-One][nxtpos-One],1,&hfdoaX_.hfpoad[ad1-One][0],1,bytes);
						lmove(&hfdoaX_.sconhf[hfdoaX_.adct-One][nxtpos-One],1,&hfdoaX_.sconhf[ad1-One][0],1,bytes);
					...
					sconX_.scon[opadriX_.sconpi[vc2loc-One]-One][8-One] = sconX_.scon[opadriX_.sconpi[vc1loc-One]-
					  One][8-One];
					sconX_.scon[opadriX_.sconpi[vc2loc-One]-One][10-
					  One] = sconX_.scon[opadriX_.sconpi[vc1loc-One]-One][10-One];
					goto L_200;
					}
				else{
					...
					if( hfdoaX_.adct <= HFPADY ){
						...
						hfdoaX_.sconhf[hfdoaX_.adct-One][2-One] = opadriX_.sconpi[vc1loc-One];
				...
				/*     No more HFDOPO positions available */

				if( diagsX_.deepdi == 1 )
					{
					fprintf( _spec_fp, " IN COPY44 - HFDOPO 65 to 74 Already Taken\n" );
					}
