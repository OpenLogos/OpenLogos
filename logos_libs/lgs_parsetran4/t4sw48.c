...
*/
	 ...
	 *              ELEMENT NN2 TO NN3.
	 *     89/07/25 LG002GBA R1957: -44002 DO NOT LOCK SUBJECT
...
void /*FUNCTION*/ t4sw48()
{
	static short int sc1, sclnk1, sclnk2, sconlc, sconp1, sconp2, temp6, temp7, xx;
	static char pgmnam[9] = "T4SW48  ";
	static short l = 0;
	static short p = 0;
	static short y = 0;
	static short z = 0;
	...
	if( diagsX_.deepdi == 1 )
		{
		fprintf( _spec_fp, " START OF SW48 %5d%5d%5d%5d%5d%5d\n", 
		         flowckX_.n6jim, n7jim, phrst1, phrls1, phrst2, phrls2 );
		for( z=0; z < 14; z++ ){
			fprintf( _spec_fp, "%4d", sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-One][z] );
			}
		fprintf( _spec_fp, "\n" );
		for( z=0; z < 14; z++ ){
			fprintf( _spec_fp, "%4d", sconX_.scon[sworkX_.phrhed[n7jim-One]-One][z] );
			}
		fprintf( _spec_fp, "\n" );
		}
	...
	if( vbdataX_.k3p1 >= 99 && vbdataX_.k3p1 <= 120 ){
		 ...
		 *        TRANSFER THE ADDRESSES IN THE VC (K3P1) IN PHRASE K3P2
		 *        TO THE SAME VC IN PHRASE K3P3 */
...
L_7335:
		...
		formsaX_.formsv[sconX_.scolnk[sworkX_.swork[flowckX_.n6jim-One][4-One]-One]-One] = semargX_.k3p3;
		}
	else if( vbdataX_.k3p1 == 11 ){

		/*     FUNCTION 11  P.R.1280
		 *     SET SCON 3 ACCORDING TO THE VALUE HELD BY K3P2 */
		...
		sconX_.scon[sworkX_.phrhed[n7jim-One]-One][3-One] = semargX_.k3p3;
		}
		...
		for( gb=phrst1; gb <= phrls1; gb++ ){
			...
			if( sconX_.scon[gbx-One][1-One] >= 0 )
		...
		if( srcflgX_.srcflg == 1 ){
			/*        COMPUTE SCON5 FOR ALL NOUNS IN THE PHRASE BASED ON THE VALUE
			...
			for( ms1=phrst1; ms1 <= phrls1; ms1++ ){
				...
				sc1 = sconX_.scon[opadriX_.sconpi[ms1-One]-One][0];
				om = 0;
				...
				if( sc1 > 0 ){
					if( (((sc1 == 1 || sc1 == 5) || sc1 == 6) || sc1 == 7) || sc1 == 8 ){
						om = nounsX_.gernum[semargX_.k3p3-One][formsaX_.formsv[sconX_.scolnk[sconX_.scon[opadriX_.sconpi[ms1-One]-
						  One][10-One]-One]-One]-One];
						if( om >= 1 && om <= 99 )
		...
		temp6 = sconX_.scolnk[sworkX_.swork[flowckX_.n6jim-One][4-One]-One];
		hensavX_.henum2[temp7-One][0] = hensavX_.henum2[temp6-One][0];
		hensavX_.henum2[temp7-One][1] = hensavX_.henum2[temp6-One][1];
		hensavX_.root_henum2[temp7-One][0] = hensavX_.root_henum2[temp6-One][0];
		hensavX_.root_henum2[temp7-One][1] = hensavX_.root_henum2[temp6-One][1];
		hashX_.hashcd[temp7-One][0] = hashX_.hashcd[temp6-One][0];
		hashX_.hashcd[temp7-One][1] = hashX_.hashcd[temp6-One][1];
		hashX_.root_hashcd[temp7-One][0] = hashX_.root_hashcd[temp6-One][0];
		hashX_.root_hashcd[temp7-One][1] = hashX_.root_hashcd[temp6-One][1];
		}
...
L_7336:
		...
		for( x1=phrst1; x1 <= phrls1; x1++ ){
			if( !(opadriX_.opadri[x1-One] < -115 || opadriX_.opadri[x1-One] > -110) ){
				if( !(opadriX_.opadri[x1-One] == -111 || opadriX_.opadri[x1-One] == -114) ){
...
L_7337:
						if( hpdopiX_.hfdopi[x1-One] >= HFPOLO && hpdopiX_.hfdopi[x1-One] <= HFPOHI ){
							...
							if( y != 0 ){
								...
								for( l=1; l <= y; l++ ){
									...
									if( !((srcflgX_.srcflg == 1 && 
									  hfdoaX_.hfpoad[h1-One][0] >= -120) &&
									  hfdoaX_.hfpoad[h1-One][0] <= -100) ){
										hfdoaX_.hfpoad[h2-One][0] = hfdoaX_.hfpoad[h1-One][0];
										om = hfdoaX_.sconhf[h1-One][0];
							...
							if( sconX_.scon[s2-One][1-One] > 0 )
								sconX_.scon[s2-One][1-One] = -sconX_.scon[s2-One][1-One];
							if( vbdataX_.k3p1 == 3 ){
								for( l=1; l <= 14; l++ ){
...
L_7338:
		...
		for( x2=phrst2; x2 <= phrls2; x2++ ){
			...
			s2 = opadriX_.sconpi[x2-One];
			if( sconX_.scon[s2-One][1-One] >= 0 ){
				if( hpdopiX_.hfdopi[x2-One] >= HFPOLO && 
					hpdopiX_.hfdopi[x2-One] <= HFPOHI ){
					...
					sconX_.scon[s2-One][4-One] = sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-
					  One][4-One];
					sconX_.scon[s2-One][6-One] = sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-
					  One][6-One];
					}
				}
			}
		...
		if( srcflgX_.srcflg == 1 ){
			...
			if( om != 0 ){
				...
				/*   NOW LOCK THE SUBJECT AND MAKE THE NUM OF THE VERB AGREE. */
				for( x1=phrst1; x1 <= phrls1; x1++ ){
					if( sconX_.scon[opadriX_.sconpi[x1-One]-One][1-One] >= 0 )
						sconX_.scon[opadriX_.sconpi[x1-One]-One][5-One] = om;
					}
		...
		if( sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-One][1-One] < 0 ){
			for( x1=phrst1; x1 <= phrls1; x1++ ){
				s1 = opadriX_.sconpi[x1-One];
				if( sconX_.scon[s1-One][0] == 1 )
				{
			if(form48_check(formsaX_.formsv[sconX_.scolnk[sconX_.scon[s1-One][10-One]-One]-One]) == 0) goto L_6760;
				}
			}
			x1 = phrls1;
			for( y1=phrst1; y1 <= x1; y1++ ){
				s1 = opadriX_.sconpi[y1-One];
				if( sconX_.scon[s1-One][1-One] >= 0 ){
					sconX_.scon[s1-One][5-One] = sconX_.scon[sworkX_.phrhed[n7jim-One]-One][5-One];
					sconX_.scon[s1-One][1-One] = -sconX_.scon[s1-One][0];
					}
			...
			if(form48_check( formsaX_.formsv[sconX_.scolnk[sworkX_.swork[flowckX_.n6jim-One][4-One]-One]-One]) == 0) goto L_6760;
			if( sconX_.scon[sworkX_.phrhed[n7jim-One]-One][5-One] != 0 ){
				for( x1=phrst1; x1 <= phrls1; x1++ ){
					s1 = opadriX_.sconpi[x1-One];
					/*+                                                        *R1957*GBA
					 *CC   IF (S1 .NE. PHRHED(N6JIM)) GOTO 6720
					 *CC   IF (SCON(1,S1) .GT. 0) SCON(1,S1) = -SCON(1,S1) */
					if( sconX_.scon[s1-One][1-One] > 0 ){
						sconX_.scon[s1-One][5-One] = sconX_.scon[sworkX_.phrhed[n7jim-One]-One][5-One];
						if( s1 == sworkX_.phrhed[flowckX_.n6jim-One] ){
							if( x1 > phrst1 && sconX_.scon[s1-1-One][1-One] ==14 )
								sconX_.scon[s1-1-One][1-One] = -sconX_.scon[s1-1-One][1-One];
							}
						}
		...
		fprintf( _spec_fp, " ILLEGAL FIRST PARAMETER,SW48 %4d\n", 
		  vbdataX_.k3p1 );
		}
