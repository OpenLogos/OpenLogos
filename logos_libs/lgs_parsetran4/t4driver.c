...
*/
	/*     T4DRIVER FORTRAN (T4DRIVER SYSLIN)
	 *     DRIVER PROGRAM FOR TRAN4 MODULE */
	/* CHANGES:
	 ...
	 *          THEN FAKE A VTR LOAD AND CONTINUE.
	 *      09/12/91 JAL: CHANGES PROCESS FOR ENTERING A NEW CLAUSE.
	 ...
	 *  04/22/86 18:27 *B0395DSD - AFTER VTRPRO, K7M CAN BE ANY
...
int rulestatfile;
int rulestat[20000];
char *rulestatfilename = "e:\\esense\\bin\\rulestat4";
#endif /* TRANSTAT */

int t4driver(void)
{
	static short int jctmnp, junk, locflg, lrgjob, retflg, retsw, srcflg, tempid, wc50sv, wc50to, ww, x, yy;
	static long int _l0, movlen;
	static short zero = 0;
	static char modnam[9] = "TRAN4   ";
	static char pgmnam[9] = "T4DRIVER";
	static short iz = 0;
	static short kz = 0;
	static short xx = 0;
	static short zz = 0;
   ...
   char lpszBuf[MAX_FILEPATH_LEN]; 
	...
	errvrsX_.untcnt = 0;
	sentctX_.sentct = 0;
	strcpy(errvrsX_.mod, "TRAN4   ");
	lrgjob = 1;

	blk_data();
	getProgramName(passesX_.passfl, passesX_.passct, 4, pgmName);
	errvrsX_.err = jcloadc(pgmName);
	if (errvrsX_.err == 0)
   {
      if (passesX_.passfl == 1 && passesX_.passct == 1)
      {
         GetConfigData("tempfile", "parse4_diag", lpszBuf, MAX_FILEPATH_LEN);
         _spec_fp = fopen(lpszBuf, "w");
      }
      else
      {
         GetConfigData("tempfile", "tran4_diag", lpszBuf, MAX_FILEPATH_LEN);
         _spec_fp = fopen(lpszBuf, "w");
      }
...
//cat-102			else if( rule_load((short*)ADR(_l0,4),&miniflX_.minifl) == 0 ){
			else if( rule_load(4, miniflX_.minifl,
						passesX_.passfl, passesX_.passct) == 0 ){

				/*                      GER SRC REPLACE DATAED NEGWC2
				 *                      (ENG SRC) WITH GERMAN VERSION */
				if( srcflg == 1 )
					memcpy(&neg4X_,&gneg4X_,sizeof(neg4X_));
				...
				if( tran4_io(ADR(_l0,0)) != 0 ){
					errvrsX_.errlvl = 1;
					/*					open the output written by this component */
					}
				else if( tran4_io(ADR(_l0,10)) == 0 ){
					goto L_5;
					}
				else{
					errvrsX_.errlvl = 1;
...
*/
		...
		/* ---------------------  DO WE WANT TO BYPASS TRAN4? ----------------- */

		/*        BYPAS ACTIVE IF TRAN1 OUTPUT IS ONLY 3 SWORKS:  BOS  WC1  EOS
		 *        MOVE INCOMING COMMONS TO OUTGOING COMMONS, JUMP TO T4WRITE() */
	...
	while( flowckX_.i3save <= sworkX_.phct ){
		...
		if( opswX_.sw[20-One] == 1 ){
			...
			for( kz=1; kz <= opadroX_.opo; kz++ ){
				fprintf( _spec_fp, "%6d", hpdopoX_.hfdopo[kz-One] );
				}
			fprintf( _spec_fp, "\n" );
			fprintf( _spec_fp, " SCONP4 " );
			for( kz=1; kz <= opadroX_.opo; kz++ ){
		...
		if( flowckX_.i3str > clsnfoX_.clndns[clsnfoX_.clcrnt-One] ){
			...
			if( tempid == clsnfoX_.clcrnt ){
				...
				/*                       MOPUP AND RESET FOR NEW CLAUSE */
				}
			else{
				...
				/*                       SET ANTECEDENT PTRS, ASSUME ALL ANTS ARE
				 *                       CONCATENATED SO JUST INIT CLANSW(). */
				...
				clsoutX_.clbgno[clsnfoX_.clcrnt-One] = sworkoX_.phcto + 
				  1;
				/*                    EACH NEW CLAUSE WILL BE ONE OUTPUT PHRASE. PHRASE
				 *                    BOUNDS ARE SET NOWHERE ELSE.  WITH NO -20 SW TOO
				 *                    DIFFICULT TO KNOW PHRASE BOUNDARIES WITHIN CLAUSES. */
		...
		getsp_x(&diacb4X_.k7, &diacb4X_.oflad, sp4yX_.ovrflw);
		if( errvrsX_.errlvl != 0 ){
		...
		else if( diacb4X_.k7 == 0 && minickX_.k7m == 0 ){
			xx = flowckX_.i3str;
			if( vtrs42X_.sw42n == 1 )
				xx = vwarg1X_.isave;
			...
			while( TRUE ){
				...
				while( TRUE ){
					vtrpre(&diacb4X_.k7,&diacb4X_.oflad,sp4yX_.ovrflw);
					...
					if( opswX_.sw[20-One] == 1)
						{
						fprintf( _spec_fp, "          %4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d%4d\n", 
						  negpas, vtrs42X_.sw42n, flowckX_.i3save, 
						  w50valX_.i3, vwarg2X_.i, vtrptrX_.nptp, 
						  sworkoX_.phcto, phsupX_.phcts, sw18bkX_.phrnew, 
						  sw36bkX_.rel, sw36bkX_.relcas, sw14bkX_.case_, 
						  sw14bkX_.sw14n, phsupX_.phsup, sw26nX_.sw26n, 
						  minickX_.k7m, i3x, vwarg1X_.isave, vwarg2X_.wc50m );
						}


					/*        THE VAL INDICATES THE SWORK TO BE USED */

					if( vtrs42X_.sw42n != 1 )
						flowckX_.n6 = flowckX_.im1 + sploopX_.li;

					/*         DEACTIVATE THIS STATEMENT BECAUSE 'PHCTO' COULD BE ZERO */

					if( opswX_.sw[20-One] == 1 ){
						fprintf( _spec_fp, "                              AT1140 - N6  LI  I  I3  PHCTO  PHCT: %5d%5d%5d%5d%5d%5d\n", 
						  flowckX_.n6, sploopX_.li, vwarg2X_.i, 
						  w50valX_.i3, sworkoX_.phcto, sworkX_.phct );
					 ...
					 *                      IMPOSSIBLE TO PROCESS THE REST OF THE SENTENCE; G
					 *                      TO THE TR2WRT ROUTINE TO SAVE WHAT WE CAN. */
					...
					if( !(minickX_.k7m != 0 && diacb4X_.k7 == 0)
					   )
						break;
					/*-                                               04/22/86  *B0395DSD */
					}
...
L_10006:
	...
	/*                  FINAL CLEANUP BEFORE WRITING OUTPUT */
	mopup();
	goto L_9900;

L_10002:
	movlen = 2*OPADRX + 2 + 2*OPADRX;
	lmove(opadroX_.opadro,1,opadriX_.opadri,1,movlen);
	movlen = 2*OPADRX;
	lmove(hpdopoX_.hfdopo,1,hpdopiX_.hfdopi,1,movlen);

	if( diagsX_.longdi == 1 )
		{
		fprintf( _spec_fp, " OPADRI ON BYPAS\n " );
		for( zz=1; zz <= opadroX_.opo; zz++ ){
			fprintf( _spec_fp, "%6d", opadroX_.opadro[zz-One] );
			}
		fprintf( _spec_fp, "\n" );
		}

L_9900:
//     write output for the sentence...
	if( tran4_io(ADR(_l0,12)) == 0 )
		goto L_9905;
	errvrsX_.errlvl = 2;
	goto L_9999;
...
L_10001:
	if( retsw == 99 ){
		...
		if( tran4_io(ADR(_l0,3)) != 0 ){
			}
		if( tran4_io(ADR(_l0,13)) != 0 ){
			}

//cat-102 two lines added
      addMemoryState("t4 driver - LSPREADCLEAN()");
//      LSPREADCLEAN();
//	Now called from TranslCommonObjects::CleanupCommonObjects()
      addMemoryState("t4 driver - rule_unload()");
	  rule_unload();
      addMemoryState("t4 driver - freeTrObjects()");
	  freeTrObjects();
      addMemoryState("t4 driver - complete");
      if (_spec_fp)
      {
         fclose(_spec_fp);
      }
