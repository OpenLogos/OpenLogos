...
void /*FUNCTION*/ t4sw46()
{
	 ...
	 *      IF (SW3146.EQ.1 .AND. (PASSFL.NE.1 .OR. PASSCT.NE.1)) GOTO 1200 */
	...
	/*   IN CASE SEMTRAN HAS MADE A CHANGE VIA SW46 WHICH SHOULD
	 *   BE COMMUNICATED TO NSWRK2 */

	/*     DO 5900 XX = 1,5
	 *     IF (SEM46(1,XX) .EQ. 0) GOTO 5920
	 *     IF (SEM46(1,XX) .NE. N6JIM) GOTO 5900
	 *     IF (SEM46(2,XX) .NE. 0) K3P2 = 0
	 *     IF (SEM46(3,XX) .NE. 0) K3P3 = 0
	 *     IF (SEM46(4,XX) .NE. 0) K3P4 = 0
	 *     GOTO 5920
	 *5900  CONTINUE
	 *-                                                     *03/19/90*JAL* */
	...
	if( semargX_.k3p3 != 0 ){
		if( semargX_.k3p3 <= 0 ){
			...
			if( sw3146X_.sw3146 == 1 ){
				...
				sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-One][11-
				  One] = sconX_.scon[sworkX_.phrhed[n7jim-One]-One][11-One];
				sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-One][13-
				  One] = sconX_.scon[sworkX_.phrhed[n7jim-One]-One][13-One];
				}
			...
			if( sworkiX_.sworki[swipos-One][2-One] < 16 && sworkiX_.sworki[swipos-One][2-One] <= 99 )
				sworkiX_.sworki[swipos-One][2-One] = semargX_.k3p3;
			sconinX_.sconin[scipos-One][2-One] = semargX_.k3p3;
			if( sconinX_.sconin[scipos-One][3-One] > 16 && sconinX_.sconin[scipos-One][3-One] <= 99 )
				sconinX_.sconin[scipos-One][3-One] = semargX_.k3p3;
			}
		else{
			if( sworkX_.swork[flowckX_.n6jim-One][2-One] < 16 && sworkX_.swork[flowckX_.n6jim-One][2-One] <= 99 )
				sworkX_.swork[flowckX_.n6jim-One][2-One] = semargX_.k3p3;
			sconX_.scon[sworkX_.phrhed[flowckX_.n6jim-One]-One][11-One] = semargX_.k3p3;
	...
	if( k3p4 < 0 ){
		...
		if( sw3146X_.sw3146 == 1 ){
			...
			if( vbdataX_.k3p1 == k3p2 && vbdataX_.k3p1 == semargX_.k3p3 )
