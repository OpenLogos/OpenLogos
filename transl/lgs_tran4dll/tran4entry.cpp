...
void lgstran4pass(const CommunicationInterface * iCommInterface, const QHandle * iQHandle,
				  short iTargetId)
{
	passesX_.passfl = 1;
	passesX_.passct = 2;
	memcpy(passesX_.pssnam,"parse ",6);
        tran4run(const_cast<CommunicationInterface *>(iCommInterface),
                 const_cast<QHandle *>(iQHandle),
                 iTargetId);
}

void tran4passcleanup(void)
{
  Tran4_IO_Cleanup();
}
