...
int lgsparsetran2(const CommunicationInterface * iCommInterface, const QHandle * iQHandle,
				   short iTargetId, bool pass2flag)
{
	...
	return tran2run(const_cast<CommunicationInterface *>(iCommInterface),
                        const_cast<QHandle *>(iQHandle),
                        iTargetId);
}

void tran2cleanup(void)
{
   Tran2_IO_Cleanup();
}
